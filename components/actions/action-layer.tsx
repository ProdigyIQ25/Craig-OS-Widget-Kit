...
const errorCopy:Record<string,string>={VALIDATION_ERROR:"Review the highlighted information and try again.",UNAUTHORIZED_ACTION:"This action is not available right now. Nothing was saved.",UPSTREAM_UNAVAILABLE:"The connection is unavailable. Nothing was saved.",WRITE_FAILED:"Could not capture this record. Nothing was saved.",DUPLICATE_REQUEST:"This capture was already submitted. Nothing new was saved.",RATE_LIMITED:"Capture is temporarily paused to prevent accidental activity. Try again shortly."};
...
export function ActionLayer({children}:{children:ReactNode}){
  ...
  return <ActionContext.Provider value={value}>{children}{definition?<div className="action-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)close()}}><section className="action-sheet" role="dialog" aria-modal="true" aria-labelledby="action-title" data-action-id={definition.id}>
    <header><div><p className="os-eyebrow">QUICK CAPTURE</p><h2 id="action-title">{definition.displayName}</h2></div><button type="button" className="action-close" aria-label="Close capture" onClick={close} disabled={result.state==="submitting"}>×</button></header>
    {result.state==="success"?<div className="action-success" role="status"><span>✓</span><h3>{definition.shortLabel} captured.</h3><p>Your record is ready when you want to add more detail.</p><div className="action-sheet-actions"><button type="button" onClick={close}>Done</button>{result.recordUrl?<a href={result.recordUrl} target="_blank" rel="noreferrer">Manage record ↗</a>:null}</div></div>:<form onSubmit={submit}>
      <p className="action-guidance">Capture the essential detail now. You can complete the record whenever you need to.</p>
