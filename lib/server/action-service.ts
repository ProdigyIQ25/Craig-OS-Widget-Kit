...
export async function executeAction(request:Request,actionId:ActionId){
  const started=Date.now();let code="WRITE_FAILED";
  try{
    ...
    if(error instanceof NotionWriteError){code=error.message.includes("configured")?"UPSTREAM_UNAVAILABLE":"WRITE_FAILED";return {status:503,body:failure(actionId,code as ActionErrorCode)}}
    ...
    console.info(JSON.stringify({event:"craig_os_action",actionId,code,durationMs:Date.now()-started,requestMarker:marker}));
  }
}
