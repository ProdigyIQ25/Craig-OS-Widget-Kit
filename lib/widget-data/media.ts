...
export type ProviderKind="youtube"|"spotify"|"vimeo"|"loom"|"direct-audio"|"external"|"unavailable";
export type MediaItem={id:string;title:string;mediaType:string;platform:string;url:string|null;mode:string;purpose:string;duration:number|null;favorite:boolean;provider:ProviderKind;embedUrl:string|null};
...
export function providerFor(raw:string|null|undefined):{provider:ProviderKind;safeUrl:string|null;embedUrl:string|null}{
  if(!raw)return {provider:"unavailable",safeUrl:null,embedUrl:null};let url:URL;try{url=new URL(raw)}catch{return {provider:"unavailable",safeUrl:null,embedUrl:null}}if(url.protocol!=="https:")return {provider:"unavailable",safeUrl:null,embedUrl:null};
...
export function deriveMedia(rows:NotionRow[],filters:{modes:MediaMode[];mediaTypes:MediaType[];favorite:boolean|null}):MediaItem[]{return rows.map(row=>{const url=providerFor(String(value(row,"URL")??""));return {id:row.id,title:textValue(row,"Title","UNTITLED MEDIA"),mediaType:textValue(row,"Media Type","UNKNOWN"),platform:textValue(row,"Platform","UNSPECIFIED"),url:url.safeUrl,mode:textValue(row,"Mode","GENERAL"),purpose:textValue(row,"Purpose",""),duration:typeof value(row,"Duration")==="number"?value(row,"Duration") as number:null,favorite:value(row,"Favorite")===true,provider:url.provider,embedUrl:url.embedUrl}}).filter(item=>filters.modes.includes(item.mode as MediaMode)&&(filters.mediaTypes.length===0||filters.mediaTypes.includes(item.mediaType as MediaType))&&(filters.favorite===null||item.favorite===filters.favorite)).sort((a,b)=>Number(b.favorite)-Number(a.favorite)||a.title.localeCompare(b.title));}
