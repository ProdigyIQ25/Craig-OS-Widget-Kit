...
function row(values:Record<string,string|number|boolean|null>):NotionRow{
  ...
  for(const [name,entry] of Object.entries(values)){
    ...
    else if(name==="Purpose"||name==="Related Context")properties[name]={type:"rich_text",rich_text:entry===null?[]:[{plain_text:String(entry)}]};
...
test("provider classification covers every authorized provider and safe fallback",()=>{
  ...
  assert.equal(providerFor("https://media.example.com/course.mp4").provider,"direct-video");
  assert.equal(providerFor("https://media.example.com/lesson.webm?download=1").provider,"direct-video");
  ...
  assert.equal(providerFor("javascript:alert(1)").provider,"unavailable");
  assert.equal(providerFor("https://user:secret@example.com/private.mp4").provider,"unavailable");
...
test("P11 derivation handles zero, multiple, favorite, missing URL, and archived-only equivalent",()=>{
  ...
  const rows=[
    row({Title:"B",URL:"https://example.com/b",Mode:"DEEP WORK","Media Type":"VIDEO",Platform:"WEB",Purpose:"Focus","Related Context":"Current objective",Duration:20,Favorite:false}),
  ...
  assert.equal(all[1].relatedContext,"Current objective");
